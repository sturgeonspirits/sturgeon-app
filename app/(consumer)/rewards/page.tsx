    ...
    <div className="p-4 max-w-lg mx-auto space-y-6">
      <div className="pt-4">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Your balance: <span className="text-[#f5c842] font-bold">{balance.toLocaleString()} pts</span>
        </p>
      </div>
      ...
      {(myRedemptions ?? []).filter(r => r.status === 'pending').length > 0 && (
        <section>
          <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Ready to Redeem</h2>
          <div className="space-y-2">
            {myRedemptions!.filter(r => r.status === 'pending').map(r => (
              <div key={r.id} className="bg-[#f5c842]/10 border border-[#f5c842]/30 rounded-xl p-4 flex items-center gap-3">
                <span className="text-2xl">{(r.rewards as any)?.icon}</span>
                <div className="flex-1">
                  <p className="font-semibold text-white text-sm">{(r.rewards as any)?.name}</p>
                  <p className="text-xs text-gray-400">Show this to staff to claim</p>
                </div>
      ...
      {/* All rewards */}
      <section>
        <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">All Rewards</h2>
        <div className="space-y-3">
          {(rewards ?? []).map(reward => {
            const canAfford = reward.redemption_method === 'points' ? balance >= reward.points_cost : null
            return (
              <div key={reward.id} className="bg-[#1a1a1a] border border-[#2e2e2e] rounded-xl p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-600 mt-1">{reward.reward_value}</p>
                    <div className="flex items-center justify-between mt-3">
                      <span className="text-xs bg-[#2e2e2e] text-gray-400 px-2 py-0.5 rounded-full">
  ...
  return (
    <form action="/api/rewards/redeem" method="POST">
      ...
      <button
        type="submit"
        disabled={!canAfford}
        className="text-xs font-bold px-3 py-1.5 rounded-lg bg-[#f5c842] text-black disabled:opacity-30 disabled:cursor-not-allowed"
      >
