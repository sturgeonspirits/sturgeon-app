...
export default function EventSignupForm({
  periodId, eventName, eventIcon, eventDate,
  participantType, teams, myTeam: initialMyTeam, isRegistered: initialIsRegistered,
}: Props) {
  ...
  // ── Team join / create ────────────────────────────────────────────────────
  async function handleJoin(permanentTeamId: string, teamName: string) {
    setLoading(true)
    setError('')
    // If the id is a name:: fallback, treat it as create-team-direct (upserts permanent_team)
    const isNameFallback = permanentTeamId.startsWith('name::')
    const endpoint = isNameFallback ? '/api/join/create-team-direct' : '/api/join/team-direct'
    const body = isNameFallback
      ? { periodId, teamName }
      : { periodId, teamId: permanentTeamId }
  ...
  async function handleLeave() {
    ...
    setLoading(false)
    if (!res.ok) { setError('Could not cancel sign-up'); return }
    setMyTeam(null)
    router.refresh()
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {participantType === 'individual' && (
        isRegistered ? (
          <div className="bg-[#87A67F]/15 border border-[#87A67F] rounded-2xl p-4 flex items-center justify-between">
            <div>
              <p className="text-xs text-[#5a7a54] uppercase tracking-widest font-medium mb-1">You&apos;re signed up</p>
              <p className="text-sm text-[#242622]">See you there!</p>
            </div>
            <button
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-[#7E613F] mt-0.5">
                      {team.memberCount === 0 ? 'No members yet' : team.members.join(', ')}
                    </p>
                  </div>
                  <button
                    onClick={() => handleJoin(team.permanentTeamId, team.name)}
                    disabled={loading}
